...
// 👇 Ajouté : inclure UI dans les sources compilées
const NUCLEAR_UI_DIR = path.resolve(__dirname, '..', '..', 'node_modules', '@nuclear', 'ui');
...
module.exports = (env) => {
  ...
  const jsxRule: webpack.RuleSetRule = {
    test: /\.(js|jsx|tsx|ts)$/,
    loader: IS_PROD ? 'ts-loader' : 'babel-loader',
    // ✅ Inclure les fichiers à compiler
    include: [APP_DIR, ...NUCLEAR_MODULES, NUCLEAR_UI_DIR],
    // ✅ Exclure tout sauf @nuclear/ui dans node_modules
    exclude: /node_modules\/(?!@nuclear\/ui).*/,
    options: IS_PROD
      ? {
      ...
      : {
          cacheDirectory: true,
          presets: [
            ['@babel/preset-env', { targets: { electron: '4.2' } }],
            '@babel/preset-react',
